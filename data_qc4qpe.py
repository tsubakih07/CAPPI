...
def gatefilter_only_rain_swp(radar,ml=4000.):
    ### data masking
...
def gatefilter_only_rain_VS(radar,ml=4000.):
    gatefilter = pyart.correct.GateFilter(radar)
    ### below melting level < 5.0-km (data from raindrop only)    
    ### extract below melting level < 5.0-km (data from raindrop only)
    for s in range(radar.nsweeps):
        _, _, alt = radar.get_gate_lat_lon_alt(s)
        if s ==0:
            alt_data = alt
        else:
            alt_data = np.append(alt_data,alt,axis=0)
            
    alt_dict = pyart.config.get_metadata(radar)
    alt_dict['data'] = alt_data
    radar.add_field('altitude',alt_dict)
    gatefilter.exclude_above('altitude',4000.)
    radar.fields.pop('altitude')    # delete field
    return gatefilter
    

def calc_sigma_swp(radar):
...
def qc_A(radar,swp=0):
    ...
    gatefilter = gatefilter_only_rain_VS(radar)
    ### remove noise
    gatefilter.exclude_below('cross_correlation_ratio', 0.85)
    
    sigma = calc_sigma_VS(radar)    
...
#%%
def get_kdp_maesaka(radar):
    # check if QC done
    if 'cor_Zh' in radar.fields.keys()== False: 
        radar,_ = qc_all(radar)
        
    ### retrive Kdp (takes about 6 min for 1 volume scan)
    kdp, _, _ = pyart.retrieve.kdp_maesaka(radar,kdp_field='specific_differential_phase')
    # add Kdp field into radar data
